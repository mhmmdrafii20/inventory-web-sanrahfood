...
export default function resep() {
    ...
    const { resep, searchRes } = usePage<{ resep: { idResep: number, namaResep: string, deleted_at: string, produk: { namaProduk: string } }[], searchRes: { idResep: number, namaResep: string, deleted_at: string, produk: { namaProduk: string } }[] }>().props;
    const { patch } = useForm()

    console.log(resep);

    function handleSearch() {
        router.get(`/resep/trash/search`, { search: searchData }, {
            preserveState: true,
            replace: true,
        })
    }
    const displayResep = searchRes && searchRes.length > 0
        ? searchRes
        : resep;
        ...
        <>
            ...
            <div className="flex flex-col gap-5  bg-white shadow-md rounded-md p-5">
                <div className="flex flex-row gap-3 items-center">
                    <Input variant={1} size="md" type="text" placeholder="Cari Nama Resep...." className="flex-1" value={searchData} onChange={e => setSearchData(e.target.value)} />
                            ...
                            <tr key={items.idResep}>
                                ...
                                <td className="border border-gray-300 py-3 px-5">
                                    <div className="flex flex-row gap-2 justify-center">
                                        <ActionButton type="restore" size="sm" title="Pulihkan data kembali" onClick={() => handleRestore(items.idResep)}>Restore</ActionButton>
                                    </div>
                                </td>
                            </tr>
                        )) : <tr className="border border-gray-300"><td colSpan={3} className="text-center py-4"><Paragraph size="lg">Tidak Ada Data Sampah Resep</Paragraph></td></tr>}
                    </tbody>
                </table>
            </div>
        </>
