...
export default function Index() {
  ...
  const { bahan, searchRes } = usePage<{
    bahan: { idBahanBaku: number; namaBahanBaku: string; deleted_at: string }[]
    searchRes: { idBahanBaku: number; namaBahanBaku: string; deleted_at: string }[]
  }>().props
  const { patch } = useForm()
            ...
            {displayBahan?.length > 0 ? (
                  ...
                  <td className="border border-gray-300 py-3 px-5">
                    <div className="flex flex-row gap-2 justify-center">
                      <ActionButton
                        ...
                        onClick={() => handleRestore(items.idBahanBaku)}
                      >
                        Restore
                      </ActionButton>
                    </div>
              ...
              <tr className="border border-gray-300">
                <td colSpan={7} className="text-center py-4">
                  <Paragraph size="lg">Tidak Ada Data Sampah Bahan Baku</Paragraph>
                </td>
              </tr>
            )}
          </tbody>
