...
export default function HakAkses() {
  ...
  const { role, searchRes } = usePage<{
    role: { idHakAkses: number; namaHakAkses: string; deleted_at: string }[]
    searchRes: { idHakAkses: number; namaHakAkses: string; deleted_at: string }[]
  }>().props
  const { patch } = useForm()
            ...
            {displayRole?.length > 0 ? (
                  ...
                  <td className="border border-gray-300 py-3 px-5">
                    <div className="flex flex-row gap-2 justify-center">
                      <ActionButton
                        ...
                        onClick={() => handleRestore(items.idHakAkses)}
                      >
                        Restore
                      </ActionButton>
                    </div>
              ...
              <tr className="border border-gray-300">
                <td colSpan={7} className="text-center py-4">
                  <Paragraph size="lg">Tidak Ada Data Sampah Hak Akses</Paragraph>
                </td>
              </tr>
            )}
          </tbody>
