...
export default function Kategori() {
  ...
  const { kategori, errors, searchRes } = usePage<{
    kategori: { idKategori: number; namaKategori: string }[]
    searchRes: { idKategori: number; namaKategori: string }[]
  }>().props
  const {
    data,
    setData,
              ...
              {displayKategori?.length > 0 ? (
                    ...
                    <td className="border border-gray-300 py-3 px-5">
                      <div className="flex flex-row gap-2 justify-center">
                        ...
                        <ActionButton
                          type="delete"
                          size="sm"
                          onClick={() => handleDelete(items.idKategori)}
                        >
                          <FaTrash />
                        </ActionButton>
                      </div>
                ...
                <tr>
                  <td colSpan={2} className="text-center py-4">
                    <Paragraph size="lg">Tidak Ada Kategori Produk</Paragraph>
                  </td>
                </tr>
              )}
            </tbody>
