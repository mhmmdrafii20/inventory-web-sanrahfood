...
export default function kategori() {
    ...
    const { kategori, searchRes } = usePage<{ kategori: { idKategori: number, namaKategori: string, deleted_at: string }[], searchRes: { idKategori: number, namaKategori: string, deleted_at: string }[] }>().props;
    const { patch } = useForm()

    function handleSearch() {
        router.get(`/kategori-produk/trash/search`, { search: searchData }, {
            preserveState: true,
            replace: true,
        })
    }
        ...
        <>
            ...
            <div className="flex flex-col gap-5  bg-white shadow-md rounded-md p-5">
                <div className="flex flex-row gap-3 items-center">
                    <Input variant={1} size="md" type="text" placeholder="Cari Nama Kategori Produk...." className="flex-1" value={searchData} onChange={e => setSearchData(e.target.value)} />
                            ...
                            <tr key={items.idKategori}>
                                ...
                                <td className="border border-gray-300 py-3 px-5">
                                    <div className="flex flex-row gap-2 justify-center">
                                        <ActionButton type="restore" size="sm" title="Pulihkan data kembali" onClick={() => handleRestore(items.idKategori)}>Restore</ActionButton>
                                    </div>
                                </td>
                            </tr>
                        )) : <tr className="border border-gray-300"><td colSpan={7} className="text-center py-4"><Paragraph size="lg">Tidak Ada Data Sampah Kategori Produk</Paragraph></td></tr>}
                    </tbody>
                </table>
            </div>
        </>
