...
export default function Index() {
  const { produk } = usePage<{
    produk: {
      ...
      namaProduk: string
      satuan: string
      resep: { idResep: number; idProduk: number; namaResep: string; jumlah: number }[]
    }[]
  }>().props
        ...
        <div className="flex flex-col gap-5">
          ...
          {selectedProduk && (
            <div className="flex flex-col gap-3">
              ...
              <Select
                ...
                name="id_resep"
                value={data.id_resep}
                onChange={(e) => setData('id_resep', e.target.value)}
              >
                <option value=" ">Pilih Resep</option>
                {selectedProduk.resep.map((items) => (
                  <option key={items.idResep} value={items.idResep}>
                    {items.namaResep}
                  </option>
                ))}
              </Select>
          ...
          <div className="flex flex-col gap-3">
            ...
            <Input
              ...
              type="number"
              name="jumlah_batch"
              placeholder="Tuliskan jumlah batch disini"
              value={data.jumlah_batch}
              onChange={(e) => setData('jumlah_batch', e.target.value)}
            />
          </div>
          ...
          <Button
            ...
            size="md"
          >
            {processing ? 'Memproses....' : 'Input Produksi'}
          </Button>
        </div>
        <div className="flex flex-col gap-5">
          <div className="flex flex-col gap-3">
