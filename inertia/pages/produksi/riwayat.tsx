...
export default function Riwayat() {
  const { riwayatProduksi, filteredRiwayatProduksi } = usePage<{
    riwayatProduksi: {
      ...
      resep: { namaResep: string; yieldPerBatch: number }
    }[]
  ...
  const displayRiwayatProduksi =
    filteredRiwayatProduksi && filteredRiwayatProduksi.length > 0
      ? filteredRiwayatProduksi
      : riwayatProduksi
      ...
      <div className="flex flex-col  w-full bg-white shadow-md rounded-md p-5">
        ...
        <div className="max-h-[500px] overflow-y-auto">
          <table className="w-full border-collapse mt-5 bg-white">
            ...
            <tbody>
              {displayRiwayatProduksi?.length > 0 ? (
                displayRiwayatProduksi?.map((items) => (
                  <tr key={items.idRiwayatProduksi}>
                    ...
                    <td className="border border-gray-300 py-3 px-5">
                      <Paragraph size="lg">
                        {dayjs(items.tanggalProduksi).format('DD/MM/YYYY')}
                      </Paragraph>
                    </td>
                    <td className="border border-gray-300 py-3 px-5">
                      <Paragraph size="lg">
                ...
                <tr className="border border-gray-300">
                  <td colSpan={7} className="text-center py-4">
                    <Paragraph size="lg">Tidak Ada Riwayat Produksi</Paragraph>
                  </td>
                </tr>
              )}
            </tbody>
