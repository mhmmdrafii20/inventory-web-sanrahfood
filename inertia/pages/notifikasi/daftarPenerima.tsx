...
export default function DaftarPenerima() {
  ...
  const { tipeNotifikasi, pengguna, errors, searchRes, daftarPenerima } = usePage<{
    tipeNotifikasi: { idTipeNotifikasi: number; kodeNotifikasi: string; namaNotifikasi: string }[]
    searchRes: {
  ...
  function handleDelete(id: number) {
    showDeleteDialog(
      ...
      'Yakin ingin menghapus daftar penerima ?',
          ...
          >
            ...
            <form className="flex flex-col gap-2 h-[600px]" onSubmit={handleCreate}>
              ...
              <Select
                ...
                name="tipe_penerima"
                value={tipePenerima}
                onChange={(e) => onTipePenerimaChange(e)}
              >
              ...
              {tipePenerima === 'Internal' && (
                <>
                  ...
                  >
                    ...
                    {pengguna?.map((items) => (
                      <option key={items.idPengguna} value={items.idPengguna}>
                        {items.namaPengguna}
                      </option>
                    ))}
                  </Select>
              ...
              <MultiSelect
                options={options}
                value={options.filter((item) => data.id_tipe_notifikasi.includes(item.value))}
                onChange={(val: Option[]) => {
                  setData(
                    'id_tipe_notifikasi',
                    val.map((item) => Number(item.value))
                  )
                }}
                labelledBy="Select"
              ></MultiSelect>
            ...
            {displayDaftarPenerima?.length > 0 ? (
              displayDaftarPenerima?.map((items, i) => (
                <tr key={items.idPenerimaNotifikasi}>
                  <td className="border border-gray-300 py-3 px-5">
                    <Paragraph size="lg">
                      {items.pengguna?.namaPengguna ?? items?.namaPenerima}
                    </Paragraph>
                  </td>
                  <td className="border border-gray-300 py-3 px-5">
                    <Paragraph size="lg">
                  ...
                  <td className="border border-gray-300 py-3 px-5">
                    <div className="flex flex-row gap-2 justify-center">
                      <Link
                        route="daftarPenerima.edit"
                        routeParams={{ id: items.idPenerimaNotifikasi }}
                      >
                        <ActionButton
                          as="div"
                      ...
                      <ActionButton
                        type="delete"
                        size="sm"
                        onClick={() => handleDelete(items.idPenerimaNotifikasi)}
                      >
                        <FaTrash />
                      </ActionButton>
                    </div>
              ...
              <tr className="border border-gray-300">
                <td colSpan={6} className="text-center py-4">
                  <Paragraph size="lg">Tidak Ada Daftar Penerima</Paragraph>
                </td>
              </tr>
            )}
          </tbody>
