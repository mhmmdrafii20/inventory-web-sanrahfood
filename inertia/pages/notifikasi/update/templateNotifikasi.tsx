...
export default function templateNotifikasi() {
    const { tipeNotifikasi, errors, dataTemplateNotifikasi, } = usePage<{ tipeNotifikasi: { idTipeNotifikasi: number, kodeNotifikasi: number, namaNotifikasi: string, templateVariables: string[] }[], dataTemplateNotifikasi: { id_template_notifikasi: number, nama_template: string, id_tipe_notifikasi: number, konten: string, tipe_notifikasi: { idTipeNotifikasi: number, kodeNotifikasi: number, namaNotifikasi: string, templateVariables: string[] } } }>().props;
    ...
    return (
        <>
            <div className="flex justify-center mx-auto mb-5">
                <Heading level={1} color="dark_slate_grey" className="font-bold">Edit Template Notifikasi</Heading>
            </div>
            <form className="flex flex-col gap-5 bg-white p-5 shadow-md rounded-md w-[600px] mx-auto" onSubmit={(handleUpdate)}>
                <Input variant={1} size="md" type="hidden" name="id_template_notifikasi" value={data.id_template_notifikasi} onChange={(e) => setData('id_template_notifikasi', Number(e.target.value))}></Input>
                <Paragraph size="lg">Tipe Notifikasi</Paragraph>
                <Select variant={1} size="md" name="id_tipe_notifikasi" value={data.id_tipe_notifikasi} onChange={(e) => setData('id_tipe_notifikasi', Number(e.target.value))}>
                ...
                <Button type="submit" variant={1} size="md" disabled={processing}>{processing ? "Updating...." : "Update"}</Button>
            </form>
        </>
    )
}