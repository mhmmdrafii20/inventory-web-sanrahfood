...
export default function TemplateNotifikasi() {
  ...
  const { tipeNotifikasi, errors, templateNotifikasi, searchRes } = usePage<{
    tipeNotifikasi: {
  ...
  const displayTemplateNotifikasi =
    searchRes && searchRes.length > 0 ? searchRes : templateNotifikasi
  ...
  function handleDelete(id: number) {
    showDeleteDialog(
      ...
      'Yakin ingin menghapus template notifikasi ?',
      ...
      <div className="flex flex-col gap-5  bg-white shadow-md rounded-md p-5">
        <div className="flex flex-row justify-between mt-5">
          ...
          >
            ...
            <form className="flex flex-col gap-2 h-[600px]" onSubmit={handleCreate}>
              ...
              <Select
                ...
                onChange={(e) => setData('id_tipe_notifikasi', e.target.value)}
              >
                ...
                {!data.id_tipe_notifikasi ? (
                  ...
                  <>
                    ...
                    <p className="mt-2 text-xs text-red-500">
                      ...
                      <code className="bg-red-100 px-1 rounded">{'{{nama_variabel}}'}</code> di
                      dalam <b>konten</b>
                    </p>
                  </>
                )}
          ...
          <div className="flex flex-row gap-3 items-center">
            <Input
              ...
              size="md"
              type="text"
              placeholder="Cari Nama Template Notifikasi...."
              className="flex-1"
              value={searchData}
        ...
        <div className="max-h-[500px] overflow-y-auto">
          <table className="w-full border-collapse mt-5 bg-white">
            ...
            <tbody>
              {displayTemplateNotifikasi?.length > 0 ? (
                displayTemplateNotifikasi?.map((items) => (
                  <tr key={items.idTemplateNotifikasi}>
                    <td className="border border-gray-300 py-3 px-5">
                      <Paragraph size="lg">{items.namaTemplate}</Paragraph>
                    </td>
                    <td className="border border-gray-300 py-3 px-5">
                      <Paragraph size="lg">{items.tipe_notifikasi.namaNotifikasi}</Paragraph>
                    </td>
                    <td className="border border-gray-300 py-3 px-5">
                      <Paragraph size="lg">{items.konten}</Paragraph>
                    </td>
                    <td className="border border-gray-300 py-3 px-5">
                      <div className="flex flex-row gap-2 justify-center">
                        <Link
                          route="templateNotifikasi.edit"
                          routeParams={{ id: items.idTemplateNotifikasi }}
                        >
                          <ActionButton
                            as="div"
                        ...
                        <ActionButton
                          type="delete"
                          size="sm"
                          onClick={() => handleDelete(items.idTemplateNotifikasi)}
                        >
                          <FaTrash />
                        </ActionButton>
                      </div>
                ...
                <tr className="border border-gray-300">
                  <td colSpan={6} className="text-center py-4">
                    <Paragraph size="lg">Tidak Ada Template Notifikasi</Paragraph>
                  </td>
                </tr>
              )}
            </tbody>
