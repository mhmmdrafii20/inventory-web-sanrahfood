...
export default function TipeNotifikasi() {
  ...
  const { errors, tipeNotifikasi, searchRes } = usePage<{
    tipeNotifikasi: {
  ...
  function handleDelete(id: number) {
    showDeleteDialog(
      ...
      'Yakin ingin menghapus tipe notifikasi ?',
      'Data akan dihapus secara permanen'
    )
  }
  function handleSearch() {
    router.get(
      `/tipe-notifikasi/search`,
      { search: searchData },
      {
        preserveState: true,
        replace: true,
  ...
  const displayTipeNotifikasi = searchRes && searchRes.length > 0 ? searchRes : tipeNotifikasi
  return (
    <>
      <Heading level={1} color="dark_slate_grey" className="font-bold">
        Manajemen Tipe Notifikasi
      </Heading>
      <div className="flex flex-col  w-full bg-white shadow-md rounded-md p-5">
        <div className="flex flex-row justify-between mt-5">
          ...
          >
            ...
            <form className="flex flex-col gap-2" onSubmit={handleCreate}>
              ...
              <MultiSelect
                ...
                value={templateVarOptions.filter((item) =>
                  data.template_variables.includes(item.value)
                )}
                onChange={(val: Option[]) => {
                  setData(
                    'template_variables',
          ...
          <div className="flex flex-row gap-5 ">
            <Input
              ...
              size="md"
              type="text"
              placeholder="Cari Nama Tipe Notifikasi"
              value={searchData}
        ...
        <div className="max-h-[500px] overflow-y-auto">
          <table className="w-full border-collapse mt-5 bg-white">
            ...
            <tbody>
              {displayTipeNotifikasi?.length > 0 ? (
                displayTipeNotifikasi?.map((items) => (
                  <tr key={items.idTipeNotifikasi}>
                    <td className="border border-gray-300 py-3 px-5">
                      <Paragraph size="lg">{items.kodeNotifikasi}</Paragraph>
                    </td>
                    <td className="border border-gray-300 py-3 px-5">
                      <Paragraph size="lg">{items.namaNotifikasi}</Paragraph>
                    </td>
                    <td className="border border-gray-300 py-3 px-5">
                      <Paragraph size="lg">{items.templateVariables.toString()}</Paragraph>
                    </td>
                    <td className="border border-gray-300 py-3 px-5">
                      <div className="flex flex-row gap-2 justify-center">
                        <Link
                          route="tipeNotifikasi.edit"
                          routeParams={{ id: items.idTipeNotifikasi }}
                        >
                          <ActionButton
                            as="div"
                        ...
                        <ActionButton
                          type="delete"
                          size="sm"
                          onClick={() => handleDelete(items.idTipeNotifikasi)}
                        >
                          <FaTrash />
                        </ActionButton>
                      </div>
                ...
                <tr>
                  <td colSpan={2} className="text-center py-4">
                    <Paragraph size="lg">Tidak Ada Tipe Notifikasi</Paragraph>
                  </td>
                </tr>
              )}
            </tbody>
