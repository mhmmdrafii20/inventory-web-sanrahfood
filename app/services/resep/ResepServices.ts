...
export class ResepServices {
    ...
    static async restore(params: number) {
        const data = await Resep.query().where('id_resep', params).update({ is_deleted: false });
        return data;
    }
    static async search(nama_resep: string) {
        return await Resep.query().where('nama_resep', 'ILIKE', `%${nama_resep}%`).where({ is_deleted: false }).preload('produk');
    }
    static async searchTrash(nama_resep: string) {
        return await Resep.query().where('nama_resep', 'ILIKE', `%${nama_resep}%`).where({ is_deleted: true }).preload('produk');
    }
}