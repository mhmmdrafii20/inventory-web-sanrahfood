...
export class BahanService {
    ...
    static async update(payload: { id_bahan_baku?: number, nama_bahan_baku?: string, satuan?: string, stok_minimum?: number }, params: number) {
        return await db.transaction(async (transaction) => {
            const data = await Bahan.query({ client: transaction }).where('id_bahan_baku', params).update({
                nama_bahan_baku: payload.nama_bahan_baku,
                satuan: payload.satuan,
            });
