...
export class TemplateNotifikasiServices {
    static async create(payload: { id_tipe_notifikasi: number, nama_template: string, konten: string }) {
    ...
    static async update(payload: { id_template_notifikasi?: number, id_tipe_notifikasi?: number, nama_template?: string, konten?: string }, params: number) {
        const tipeNotifikasi = await TipeNotifikasi.find(payload.id_tipe_notifikasi);
    ...
    static async search(search: string) {
        return await TemplateNotifikasi.query().where('nama_template', 'ILIKE', `%${search}%`).preload('tipe_notifikasi');
    }
}