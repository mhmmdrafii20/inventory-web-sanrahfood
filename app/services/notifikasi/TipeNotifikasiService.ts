...
export default class TipeNotifikasiService {
    static async create(payload: { kode_notifikasi: string, nama_notifikasi: string, template_variables: string[] }) {
        const data = await TipeNotifikasi.create({ ...payload });
        return data;
    }
    static async update(payload: { id_tipe_notifikasi?: number, kode_notifikasi?: string, nama_notifikasi?: string, template_variables?: string[] }, params: number) {
