...
export class DaftarPenerimaServices {
    static async create(payload: { id_pengguna?: string, nama_penerima?: string, nomor_telepon?: string, id_tipe_notifikasi: number[] }) {
    ...
    static async update(payload: { id_pengguna?: string, nama_penerima?: string, nomor_telepon?: string, id_tipe_notifikasi: number[] }, params: number) {
        await db.transaction(async (transaction) => {
            await PenerimaNotifikasi.query({ client: transaction }).where('id_penerima_notifikasi', params).update({
                id_pengguna: payload.id_pengguna,
                nama_penerima: payload.nama_penerima,
                nomor_telepon: payload.nomor_telepon
            })

            await PenerimaJenisNotifikasi.query({ client: transaction }).where('id_penerima_notifikasi', params).delete()
    ...
    static async search(search: string) {
        return await PenerimaNotifikasi.query()
            ...
            .where((query) => {
                query.where('nama_penerima', 'ILIKE', `%${search}%`)
