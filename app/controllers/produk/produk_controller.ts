...
export default class ProdukController {
  ...
  async destroy({ response, params, session }: HttpContext) {
    try {
      ...
      const cekProdukHasResep = await Resep.query()
  ...
  async restore({ response, params, session }: HttpContext) {
    try {
      ...
      const kategori = await Kategori.query()
        .where('id_kategori', Number(produk?.id_kategori))
        .where({ is_deleted: false })
        .first()

      if (!kategori) {
        throw new Error(
          `${produk?.nama_produk} tidak dapat dipulihkan karena kategorinya sudah dihapus.`
        )
      }
