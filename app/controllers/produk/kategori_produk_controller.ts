...
import { updateKategoriProdukValidator, kategoriProdukValidator } from '#validators/produk/kategori_produk';
...
export default class KategoriController {
    ...
    async destroy({ response, params, session }: HttpContext) {
        try {
            ...
            const cekProdukHasKategori = await Produk.query().where('id_kategori', params.id).where({ is_deleted: false }).first();

            if (cekProdukHasKategori) {
                throw new Error(`Kategori ini masih digunakan oleh produk ${cekProdukHasKategori.nama_produk}`);
            }
