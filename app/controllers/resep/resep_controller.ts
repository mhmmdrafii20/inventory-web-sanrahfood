...
export default class ResepController {
    async index({ inertia }: HttpContext) {
        ...
        const resep = await Resep.query().whereHas('produk', (produkQuery) => {
    ...
    async edit({ inertia, params }: HttpContext) {
        ...
        return inertia.render('resep/update/resep', { specificDataResep, specificResepBahan, produk, bahan });
    }
    ...
    async restore({ response, session, params }: HttpContext) {
        try {
            ...
            const produk = await Produk.query().where('id_produk', Number(resep?.id_produk)).where({ is_deleted: false }).first();

            if (!produk) {
                throw new Error(`${resep?.nama_resep} tidak dapat dipulihkan karena produk nya sudah dihapus.`);
            }
