...
export default class PenggunaController {
  ...
  async destroy({ response, session, params }: HttpContext) {
    try {
      const pengguna = await Pengguna.query().where('id_pengguna', params.id).first()
      const cekPenggunaHasPenerimaNotifikasi = await PenerimaNotifikasi.query()
        .where('id_pengguna', params.id)
        .first()
  ...
  async restore({ response, session, params }: HttpContext) {
    try {
      const pengguna = await Pengguna.query()
        ...
        .preload('hakAkses')
        .first()
      const hakAkses = await HakAkses.query()
        .where('id_hak_akses', Number(pengguna?.id_hak_akses))
        .where({ is_deleted: false })
        .first()

      if (!hakAkses) {
        throw new Error(
          `${pengguna?.nama_pengguna} tidak dapat dipulihkan karena hak aksesnya sudah dihapus.`
        )
      }
