...
export default class HakAksesController {
  ...
  async destroy({ response, params, session }: HttpContext) {
    try {
      ...
      const cekPenggunaHakAkses = await Pengguna.query()
        .where('id_hak_akses', params.id)
      ...
      if (cekPenggunaHakAkses) {
        throw new Error(
          `${cekPenggunaHakAkses?.nama_pengguna} masih menggunakan hak akses ${cekPenggunaHakAkses?.hakAkses?.nama_hak_akses}`
        )
      }
