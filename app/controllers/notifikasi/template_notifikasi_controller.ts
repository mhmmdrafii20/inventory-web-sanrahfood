...
import {
  templateNotifikasiValidator,
  updateTemplateNotifikasiValidator,
} from '#validators/notifikasi/template_notifikasi'
...
export default class TemplateNotifikasiController {
  ...
  async edit({ inertia, params }: HttpContext) {
    const templateNotifikasi = await TemplateNotifikasi.query()
      .where('id_template_notifikasi', params.id)
      .preload('tipe_notifikasi')
      .first()
    const tipeNotifikasi = await TipeNotifikasi.all()
    const dataTemplateNotifikasi = templateNotifikasi?.$attributes
    return inertia.render('notifikasi/update/templateNotifikasi', {
