...
export default class NotifikasiWhatsappController {
  async index({ inertia, user }: HttpContext) {
    ...
    const rawWhatsappSession = await NotifikasiWhatsapp.query()
      .where('id_pengguna', penggunaId)
      .first()
    const whatsappSession = rawWhatsappSession?.$attributes
    return inertia.render('notifikasi/notifikasiWhatsapp', { whatsappSession })
  }
  async createSession({ response, session, user }: HttpContext) {
    try {
      ...
      await NotifikasiWhatsappServices.createSession(zawaId, sessionId, penggunaId, status)
