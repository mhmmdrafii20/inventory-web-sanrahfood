...
import {
  tipeNotifikasiValidator,
  updateTipeNotifikasiValidator,
} from '#validators/notifikasi/tipe_notifikasi'
...
export default class TipeNotifikasiController {
  ...
  async destroy({ response, params, session }: HttpContext) {
    try {
      ...
      const cekPenerimaNotifikasiHasTipeNotifikasi = await PenerimaJenisNotifikasi.query()
        .where('id_tipe_notifikasi', params.id)
        .first()

      if (cekPenerimaNotifikasiHasTipeNotifikasi) {
        throw new Error(
          `${tipeNotifikasi?.nama_notifikasi} masih digunakan sebagai tipe notifikasi`
        )
      }
