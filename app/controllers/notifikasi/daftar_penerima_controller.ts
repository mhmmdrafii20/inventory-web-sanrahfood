...
import { daftarPenerimaValidator, updateDaftarPenerimaValidator } from "#validators/notifikasi/daftar_penerima";
...
export default class DaftarPenerimaController {
    ...
    async edit({ inertia, params }: HttpContext) {
        const daftarPenerima = await PenerimaNotifikasi.query().where('id_penerima_notifikasi', params.id).preload('pengguna', (PenggunaQuery) => {
            PenggunaQuery.where({ is_deleted: false })
        }).first()
        ...
        const specificTipeNotifikasi = await PenerimaJenisNotifikasi.query().where('id_penerima_notifikasi', params.id).preload('tipe_notifikasi');

        const dataSpecificTipeNotifikasi = specificTipeNotifikasi.map((item) => item.$attributes.id_tipe_notifikasi);

        const pengguna = await Pengguna.query().whereHas('hakAkses', (hakAksesQuery => {
            hakAksesQuery.where({ is_deleted: false })
        })).preload('hakAkses').where({ is_deleted: false });

        return inertia.render('notifikasi/update/daftarPenerima', { dataDaftarPenerima, tipeNotifikasi, dataSpecificTipeNotifikasi, pengguna })
    }
    ...
    async destroy({ response, params, session }: HttpContext) {
        try {
            const daftarPenerima = await PenerimaNotifikasi.query().where('id_penerima_notifikasi', params.id).first();
            await DaftarPenerimaServices.destroy(params.id);

            session.flash('success', `${daftarPenerima?.nama_penerima} berhasil dihapus`);
            return response.redirect().toRoute('daftarPenerima.index');
        } catch (error) {
            console.error(error)
            if (error.code === 'E_VALIDATION_ERROR') {
