...
const uniqueResep = vine.createRule(
...
export const resepValidator = vine.create({
  nama_resep: vine.string().minLength(3).maxLength(255).use(uniqueResep()).trim(),
  yield_per_batch: vine.number().min(1),
  bahan: vine
    .array(
      vine.object({
    ...
    .minLength(1),
  id_produk: vine.number().min(1),
  catatan_tambahan: vine.string().maxLength(255).trim().optional(),
})
...
export const updateResepValidator = (id: number) =>
  vine.create({
    id_resep: vine.number().min(1).optional(),
    nama_resep: vine
      .string()
      .minLength(3)
      .maxLength(255)
      ...
      .optional(),
    yield_per_batch: vine.number().min(1).optional(),
    bahan: vine
      .array(
        vine.object({
      ...
      .minLength(1),
    id_produk: vine.number().min(1).optional(),
    catatan_tambahan: vine.string().maxLength(255).trim().optional(),
  })
