...
const uniqueHakAkses = vine.createRule(async (value: unknown, _, field: FieldContext) => {
...
export const hakAksesValidator = vine.create({
    nama_hak_akses: vine
        .string().unique({ table: 'tb_hak_akses', column: 'nama_hak_akses' })
        .minLength(3)
        .maxLength(255)
        .use(uniqueHakAkses())
        .trim()
...
export const updateHakAksesValidator = (id: number) => vine.create({
    id_hak_akses: vine
        .number().min(1)
        .optional(),
    nama_hak_akses: vine
